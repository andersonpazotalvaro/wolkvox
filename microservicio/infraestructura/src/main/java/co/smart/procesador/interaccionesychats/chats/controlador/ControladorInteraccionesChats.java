package co.smart.procesador.interaccionesychats.chats.controlador;


import co.smart.procesador.interaccionesychats.chats.comando.ResponseInteraccionesChats;
import co.smart.procesador.interaccionesychats.chats.comando.manejador.ManejadorInteraccionesChats;
import co.smart.procesador.interaccionesychats.chats.consumir.ConsumirInteraccionesChats;
import co.smart.procesador.interaccionesychats.email.servicio.NotificacionInteraccionesChats;
import com.google.gson.Gson;
import okhttp3.Response;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.bind.annotation.RestController;

import java.io.IOException;

@RestController
@RequestMapping("Interaccioneschats")
public class ControladorInteraccionesChats {

        private final static String NO_SE_ENCONTRO_DATOS = "no se encontraron datos para estas fechas";

        private final NotificacionInteraccionesChats notificacionInteraccionesChats;

        private final ManejadorInteraccionesChats manejadorInteraccionesChats;

        private final ConsumirInteraccionesChats consumirInteraccionesChats;


    public ControladorInteraccionesChats(NotificacionInteraccionesChats notificacionInteraccionesChats, ManejadorInteraccionesChats manejadorInteraccionesChats, ConsumirInteraccionesChats consumirInteraccionesChats) {
        this.notificacionInteraccionesChats = notificacionInteraccionesChats;
        this.manejadorInteraccionesChats = manejadorInteraccionesChats;
        this.consumirInteraccionesChats = consumirInteraccionesChats;
    }

    @RequestMapping(value= "/{ini}/{fin}", method = RequestMethod.POST)
    public String enviar(@PathVariable("ini") String ini, @PathVariable("fin") String fin) throws IOException {

        Response response= this.consumirInteraccionesChats.consumirr(ini,fin);
        boolean guardo= false;
        Gson gson= new Gson();
        String json;
        if(response.code()!=404) {
             json = response.body().string();
            ResponseInteraccionesChats responseInteraccionesChats= gson.fromJson(json, ResponseInteraccionesChats.class);
            try {
                this.manejadorInteraccionesChats.ejecutar(responseInteraccionesChats, ini, fin);
                guardo = true;
            } catch (Exception e) {
                return e.getMessage()+"\n"+ response.code()+ "\n"+ responseInteraccionesChats.getMsg();
            }
            System.out.println("guardo");


            this.notificacionInteraccionesChats.ejecutar(guardo);
            return json;
        }
        else {
            json= NO_SE_ENCONTRO_DATOS;
            return json;
        }

    }



   }
